...
instance.interceptors.request.use(
  function (config) {
    ...
    config.headers.Authorization = `Bearer ${
      useTokenStorage.getToken()?.token ?? ""
    }`;
    return config;
  },
  function (error) {
...
instance.interceptors.response.use(
  function (response) {
    ...
    // 2xx 范围内的状态码都会触发该函数
    // 对响应数据做点什么
    const responseCustom = response as Required<
      UnResponse<HttpResponseData | string>
    >;
    const config = (responseCustom.config || {}) as CustomUnConfig;
    const returnData = responseCustom.data as HttpResponseData;
    // 刷新token
    if (
      !config.disableRefreshToken &&
      useTokenStorage.isTokenExpire(returnData._)
    ) {
      setTimeout(async () => {
        try {
    ...
    if (
      config.name === "file" &&
      config.filePath &&
      typeof responseCustom.data === "string"
    ) {
  ...
  function (error) {
    ...
    const message = transformHttpStatusMessage(
      code,
