<template>
    <view class="p-15 bg-white flex m-20 b-rd-4 min-h-270" @click="isClick && onGoPage({ name: 'room', params: { id: item?.id } })">
        <view style="flex: 2">
                ...
                <view class="flex flex-justify-between">
                    <view class="w-full">
                        ...
                        <view v-if="item?.tag?.length">
                            <text class="tag mr-10 green" style="display: inline-block" v-for="t in item.tag" :key="`${item?.id}-${t}`">{{ t }} </text>
                        </view>
                    </view>
                    ...
                    <view class="status-red w-100" v-if="item?.useStatus == 1">
                        ...
                        <view class="txt-2 text-center"> 使用中 </view>
                    </view>
                    <view class="status-primary w-130" v-else-if="item?.useStatus == 2">
                        <view class="text-center"> 仍可预订 </view>
                        <view class="txt-2 text-center"> 保洁中 </view>
                    </view>
                    <view class="status-default w-120" v-else-if="item?.useStatus == 3">
                        <view class="text-center"> 暂停预订 </view>
                        <view class="txt-2 text-center"> 维护中 </view>
                    </view>
                    <view class="status-green w-100" v-else>
                        <view class="text-center"> 可预定 </view>
                        <view class="txt-2 text-center"> 空闲中 </view>
                    </view>
                </view>
