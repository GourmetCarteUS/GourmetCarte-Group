<template>
    <view class="flex m-20 h-800 mb-10 b-rd-8 overflow-hidden min-h-250" @click="onGoPage({ name: 'hub' }, false)">
        <view class="w-400 relative overflow-hidden" :style="`background: url(${item?.picture.length ? item?.picture[0] : undefined}) center center/cover`">
        ...
        <view class="bg-white p-20 w-full flex flex-col flex-justify-between">
            ...
            <view class="text-24 flex flex-justify-between mt-10">
                <view class="room-num" :class="item?.freeNum && item?.freeNum >= Math.ceil(item?.totalNum / 2) ? 'green' : 'red'">
                    空闲{{ item?.freeNum }}/{{ item?.totalNum }}
                </view>
                <view class="text-white bg-primary room-num" @click.stop="openMap"> 导航 </view>
            </view>
        </view>
    </view>
</template>
