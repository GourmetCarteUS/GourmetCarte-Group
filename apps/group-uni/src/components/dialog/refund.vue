    ...
    <view class="popup-content p-40">
      ...
      <view class="content" v-if="isSuccess">
        ...
        <radio-group @change="radioChange">
          <label class="radio text-24">房间卫生差
            <radio value="1" color="#FBDA41" checked/>
          </label>
          <label class="radio text-24">其他原因
            <radio value="2" color="#FBDA41"/>
          </label>
        </radio-group>
        <textarea placeholder="请填写退款原因" class="text-24 mb-20" v-show="refundForm.type==2"
...
.popup-content {
  ...
  .content {
    ...
    textarea {
      border: 1px solid rgba(#938C7A, .6);
      width: 100%;
      padding: 10rpx;
      font-size: 30rpx;
    }
