...
const popupRef = ref(), couponData = reactive<{
  title?: string
  duration?: number
  price?: number
  quanCode?: string
  ...
  <uni-popup ref="popupRef" type="bottom" background-color="#fff">
    <view class="p-20">
      <view class="center relative text-34">
        团购券验券
        <CincoreIcon styles="absolute right-0" size="32px" type="guanbi"
                     @click="popupRef?.close"/>
      </view>

      <view class="center flex-col mt-40">
        <view class="flex mb-30" style="transform: scale(.5);">
        ...
        <view class="w-500">
          <view class="scan-input mb-28">
            <uni-easyinput suffixIcon="scan" v-model="couponForm.payIdentifying" placeholder="输入卡券兑换码"
                           placeholderStyle="font-size:16px" trim="all"
                           @iconClick="iconClick" :inputBorder="false"/>
          </view>
