        ...
        <view class="bg-white b-rd-20 w-600">
            <view class="center header">
                ...
                <text class="text-28 text-primary">
                    <template v-if="[1, 2, 4].includes(dialogCode)"> 温馨提示 </template>
                    <template v-else> 预订提示 </template>
                </text>
            </view>

            <view class="p-40 pb-20 text-26 body">
                <view v-if="dialogCode === 1"> 该房间正在维护中，暂时不可预定，建议您更换其他房间看看。为您带来的不便，我们深表歉意！ </view>
                <view v-if="dialogCode === 2"
                    >该房间有用户正在使用。您可订阅到时通知。我们将在本房间可预订时，通过微信“服务通知”告知您！
