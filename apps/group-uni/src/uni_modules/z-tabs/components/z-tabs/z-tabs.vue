      ...
      <scroll-view ref="z-tabs-scroll-view" class="z-tabs-scroll-view" :scroll-x="true" :scroll-left="scrollLeft"
        ...
        <view class="z-tabs-list-container" :style="[tabsListStyle]">
          <view class="z-tabs-list" :style="[tabsListStyle, {marginTop: -finalBottomSpace+'px'}]">
            <view :ref="`z-tabs-item-${index}`" :id="`z-tabs-item-${index}`" class="z-tabs-item" :style="[tabStyle]"
                  v-for="(item,index) in list" :key="index" @click="tabsClick(index,item)">
              <view class="z-tabs-item-title-container">
                ...
                <text v-if="item.badge&&_formatCount(item.badge.count).length" class="z-tabs-item-badge"
                      :style="[badgeStyle]">{{ _formatCount(item.badge.count) }}
                </text>
              </view>
            </view>
          </view>
...
export default {
  ...
  props: {
    ...
    activeColor: {
      type: String,
      default: _gc('activeColor', '#000001')
    },
    //未激活状态tab的颜色
    inactiveColor: {
      type: String,
      default: _gc('inactiveColor', 'rgba(0,0,1,0.7)')
    },
    //禁用状态tab的颜色
    disabledColor: {
      type: String,
