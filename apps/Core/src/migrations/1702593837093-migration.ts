...
export class Migration1702593837093 implements MigrationInterface {
    name = 'Migration1702593837093';

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `CREATE TABLE \`event\` (\`id\` varchar(36) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`description\` varchar(255) NOT NULL, \`geoLocation\` point NOT NULL, \`location\` varchar(255) NOT NULL, \`startAt\` datetime NOT NULL, \`title\` varchar(255) NOT NULL, \`maleTicketFee\` int NOT NULL, \`femaleTicketFee\` int NOT NULL, \`creatorId\` varchar(36) NULL, SPATIAL INDEX \`IDX_65d15547939ccd969d9c2e30ae\` (\`geoLocation\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
        );
        await queryRunner.query(
            `CREATE TABLE \`user\` (\`id\` varchar(36) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`avatarUrl\` varchar(255) NULL, \`displayName\` varchar(255) NULL, \`openId\` varchar(255) NULL, \`unionId\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
        );
        await queryRunner.query(
            `CREATE TABLE \`event_participants_user\` (\`eventId\` varchar(36) NOT NULL, \`userId\` varchar(36) NOT NULL, INDEX \`IDX_cb16f471dfa696d2da841aaf21\` (\`eventId\`), INDEX \`IDX_120333cf695db458809e8b29e2\` (\`userId\`), PRIMARY KEY (\`eventId\`, \`userId\`)) ENGINE=InnoDB`
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `ALTER TABLE \`event_participants_user\` ADD CONSTRAINT \`FK_120333cf695db458809e8b29e22\` FOREIGN KEY (\`userId\`) REFERENCES \`user\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`
        );
    }
